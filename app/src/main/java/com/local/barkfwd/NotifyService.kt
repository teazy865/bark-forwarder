...
class NotifyService : NotificationListenerService() {
    ...
    override fun onNotificationPosted(sbn: StatusBarNotification?) {
        ...
        sendParsed(pkg, group, prefs, posted.notification)
    }

    private fun sendParsed(pkg: String, group: String, prefs: Prefs, notification: Notification) {
        ...
        val icon = prefs.iconFor(pkg)
        val openUrl = prefs.openUrlFor(pkg)
        val shown = body.ifBlank { title }
        thread {
            val r = BarkClient.send(prefs.barkKey, head, shown, group, icon, openUrl, 5)
            if (r.isSuccess) prefs.addEvent(group, shown.take(48))
        }
    }
